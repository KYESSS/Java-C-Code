...
void addItem(vector<Snack>& items, Snack numberEntered) {
...
int main () {
    ...
    cout << "Current Item: " << (int) currentItem << endl;

    vector<Snack> items;
    int numberEntered = (int) Snack::DARK_CHOCOLATE;
    for (int i = 0; i < 3; i++) {
        cout << "Enter a number:";
        cin >> numberEntered;
        addItem(items, (Snack) numberEntered);
    }
}
